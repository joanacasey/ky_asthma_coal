##################################################################################################################
# Code used to conduct indiv-level analyses for "Improved asthma outcomes observed in the vicinity of coal power # 
# plant retirement, retrofit and conversion to natural gas"                                                      #
...
dati_mo <- dati_mo %>% 
  group_by(ids) %>%
  tq_transmute(select     = c(Rscu, Mold, WindSpeed, RelativeHumidity, StationPressure, Weed, Grass, Tree, SO2_Harvard, SO2PPB, time_since, DryBulbFarenheit, Mcrk_u1364_3),
               mutate_fun = apply.monthly,
               FUN        = mean)
...
#Figure out mean HyADS in per-period to Mil Creek Unit 3
dati_mo2 <- dati %>% 
...
dati_mo <- read_csv("propeller_monthly.csv")

#gnm model for continous HyADS exposure
model <- gnm(Rscu ~ SO2_Harvard + ns(RelativeHumidity,2) + ns(WindSpeed,2) +  ns(DryBulbFarenheit, 2) + ns(StationPressure, 2) +
...
model2 <- gnm(Rscu ~ SO2_Harvard + ns(RelativeHumidity,2) + ns(WindSpeed,2)  +   ns(DryBulbFarenheit, 2) +ns(StationPressure, 2) +
...
#Adjusted - Equation 5#
#######################
mod_gnm_adjust <- gnm(Rscu ~ scrub4 + t + t_scrub4 + harmonic(month, 2, 12)  + ns(StationPressure, 2) +
...
mod_gnm_adjust2 <- gnm(Rscu ~ scrub4 + t + t_scrub4  + harmonic(month, 2, 12)  + reslag1 + ns(StationPressure, 2) +
                         +ns(RelativeHumidity,2) + WindSpeed  + ns(Mold,2) + Weed + Tree + ns(DryBulbFarenheit, 2) +
                         Grass, data=dati_mo, family="quasipoisson",
                       eliminate=factor(ids))
...
dati_mo_plot <- dplyr::filter(dati_mo_plot, t<=25)
dati_mo_plot$switch <- factor(dati_mo_plot$switch, levels=c("pre", "post"), labels=c("Pre", "Post"))
...
summary(mod_gnm_unadj) 
#Adjusted
mod_gnm <- gnm(Rscu_bi ~ scrub4 + t + t_scrub4 + harmonic(month, 2, 12)  + ns(StationPressure, 2) + 
                 +ns(RelativeHumidity,2) + WindSpeed  + ns(Mold,2) + Weed + Tree + ns(DryBulbFarenheit, 2) +
                 Grass, data=dati_mo, family="binomial",
               eliminate=factor(ids))
#Using Brumback autocorrelation adjustment for high vs. not use
reslag1 <- Lag(residuals(mod_gnm,type="deviance"),1)
mod_gnm <- gnm(Rscu_bi ~ scrub4 + t + t_scrub4 + harmonic(month, 2, 12)  + reslag1 + ns(StationPressure, 2) + 
                 +ns(RelativeHumidity,2) + WindSpeed  + ns(Mold,2) + Weed + Tree + ns(DryBulbFarenheit, 2) +
                 Grass, data=dati_mo, family="binomial",
               eliminate=factor(ids))

#Binary 4+ vs. <4 on average
dati_mo$Rscu_4 <- ifelse(dati_mo$Rscu_4 > 4, 1, 0)
#Adjusted
mod_gnm <- gnm(Rscu_4 ~ scrub4 + t + t_scrub4 + harmonic(month, 2, 12)  + ns(StationPressure, 2) + 
                 +ns(RelativeHumidity,2) + WindSpeed  + ns(Mold,2) + Weed + Tree + ns(DryBulbFarenheit, 2) +
                 Grass, data=dati_mo, family="quasipoisson",
               eliminate=factor(ids))
reslag1 <- Lag(residuals(mod_gnm,type="deviance"),1)
mod_gnm <- gnm(Rscu_4 ~ scrub4 + t + t_scrub4 + harmonic(month, 2, 12)  + reslag1 + ns(StationPressure, 2) + 
                 +ns(RelativeHumidity,2) + WindSpeed  + ns(Mold,2) + Weed + Tree + ns(DryBulbFarenheit, 2) +
                 Grass, data=dati_mo, family="quasipoisson",
               eliminate=factor(ids))
summary(mod_gnm)