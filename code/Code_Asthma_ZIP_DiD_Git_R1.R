...
exp_dat5 <- exp_dat4 %>% filter(yrq.x=="2014-2" | yrq.x=="2014-3"| yrq.x=="2014-4"| yrq.x=="2015-1"| yrq.x=="2015-2"| yrq.x=="2015-3"| yrq.x=="2015-4"| yrq.x=="2016-1"| yrq.x=="2016-2")
head(exp_dat5)

##Look at change in avg HyADS from pre to post period and define exposed based on that
exp_dat5 <- exp_dat5 %>% mutate(coalclose = dplyr::case_when(( yrq.x=="2015-3"| yrq.x=="2015-4"| yrq.x=="2016-1"| yrq.x=="2016-2") ~ 1,
                                                             ( yrq.x=="2015-2" | yrq.x =="2014-2"| yrq.x =="2014-3"| yrq.x == "2014-4" | yrq.x == "2015-1") ~ 0))
...
#Look for natural break in HyADS change to define high v low
tiff(filename = "/Users/joancasey/Documents/Columbia/HyADS_Asthma/Nat_Energy/ZIP_Natural_Break_DiD2.tiff", type = "cairo" ,  res=300, height = 3.5, width = 4, units = "in")
ggplot(exp_dat5b, aes(hyads_diff)) + geom_histogram(bins=50) + theme_minimal() +
  ylab("Number of ZIP codes") + xlab("\u0394 HyADS") #-26000 looks like a natural break
dev.off()

#Create indicator at -26000
exp_dat5b <- exp_dat5b %>% mutate(exposed = case_when(hyads_diff< -26000 ~ 1,
                                                      hyads_diff >= -26000 ~ 0))

#Rescale HyADS
exp_dat5b$hyads_diff <- divide.1000(exp_dat5b$hyads_diff)*-1

exp_dat5b <- dplyr::select(exp_dat5b, hyads_diff, exposed, zip)

#Add back on to original dataframe
exp_dat5 <- left_join(exp_dat5, exp_dat5b, by = c("zip" = "zip"))

#Equal to 1 if pre-period HyADS was >=3rd tertile
exp_dat5$exposed_coalclose <- exp_dat5$exposed*exp_dat5$coalclose
exp_dat5$exposed_coalclose_c <- exp_dat5$hyads_diff*exp_dat5$coalclose
...
#Unadjusted, quarterly, binary HyADS analysis
...
#      Equation 4       #
#########################
#Adjusted, quarterly, binary HyADS analysis, Figure 3, Panel 3
...
#Sensitivity for panels [3]-[4] weighting by baseline population rather than adjusting for baseline population in the model
...
#Unadjusted, quarterly, continuous HyADS analysis, Figure 3, Panel 4
modelDiD0c <- lm(asthma_count ~ coalclose + hyads_diff + exposed_coalclose_c + totpop +  as.factor(zip) , dat=exp_dat5)
summary(modelDiD0c)
#Clustered SE @ZIP level
...
modelDiD1c <- lm(asthma_count ~ coalclose + hyads_diff + exposed_coalclose_c + totpop + uninsur_p +  qtr.f + factor(year) +
                  pov_p + black_p + RH_Quarter + Pres_Quarter + WS_Quarter + Temp_Quarter +  as.factor(zip)  , dat=exp_dat5)
summary(modelDiD1c)
#Clustered SE @ ZIP level
...
#Panel [4],  weighted
modelDiD0c <- lm(asthma_count ~ coalclose + hyads_diff + exposed_coalclose_c  +  as.factor(zip), 
                 dat=exp_dat5, weights = wts)
summary(modelDiD0c)
#Clustered SE @ZIP level
coef_test(modelDiD0c, vcov = "CR2", 
...
modelDiD1c <- lm(asthma_count ~ coalclose + hyads_diff + exposed_coalclose_c  + uninsur_p +  qtr.f + factor(year) +
                   pov_p + black_p + RH_Quarter + Pres_Quarter + WS_Quarter + Temp_Quarter +  as.factor(zip),
                 dat=exp_dat5, weights =  wts)
summary(modelDiD1c)
#Clustered SE @ ZIP level
...
#Annual change data
exp_dat5 <- exp_dat5 %>% mutate(pre = case_when(yrq.x == "2014-2" | yrq.x == "2014-3" | yrq.x == "2014-4"| yrq.x == "2015-1" ~ 1,
                                                yrq.x == "2015-3" | yrq.x == "2015-4" | yrq.x == "2016-1"| yrq.x == "2016-2" ~ 0))
...
did2 <- lm(asthma_diff ~ hyads_diff + uninsur_diff + totpop_diff + totpop +
             Temp_diff + Pres_diff, data = exp_dat8)
summary(did2)
#Clustered SE
coef_test(did2, vcov = "CR2", 
          cluster = exp_dat8$zip, test = "Satterthwaite")[1:2,]
confint(did2)
car::vif(did2)
...
#Unadjusted
did <- lm(asthma_diff ~ hyads_diff, data = exp_dat8, weights = wts)
summary(did)
#Clustered SE
coef_test(did, vcov = "CR2", 
          cluster = exp_dat8$zip, test = "Satterthwaite")[1:2,]
...
did2 <- lm(asthma_diff ~ hyads_diff + uninsur_diff + totpop_diff  +
...
did_cat <- lm(asthma_diff ~ as.factor(hyads_group) + totpop, data = exp_dat8)
summary(did_cat)
#Clustered SE
coef_test(did_cat, vcov = "CR2", 
          cluster = exp_dat8$zip, test = "Satterthwaite")[1:4,]

#########################
#      Equation 3       #
#########################
#Adjusted, annual, categorical HyADS analysis, Figure 3, Panel 2
did_cat2 <- lm(asthma_diff ~ as.factor(hyads_group) + totpop_diff + totpop + uninsur_diff + 
             Temp_diff + Pres_diff, data = exp_dat8)
summary(did_cat2)
#Clustered SE
coef_test(did_cat2, vcov = "CR2", 
          cluster = exp_dat8$zip, test = "Satterthwaite")[1:4,]

#Population weights
did_cat <- lm(asthma_diff ~ as.factor(hyads_group) , data = exp_dat8, weights = wts)
summary(did_cat)
#Clustered SE
coef_test(did_cat, vcov = "CR2", 
          cluster = exp_dat8$zip, test = "Satterthwaite")[1:4,]

did_cat2 <- lm(asthma_diff ~ as.factor(hyads_group) + totpop_diff + uninsur_diff + 
                 Temp_diff + Pres_diff, data = exp_dat8, weights = wts)
summary(did_cat2)
#Clustered SE
coef_test(did_cat2, vcov = "CR2", 
...
##Annual results##
##################
dat<-read_csv("annual_DiD_results.csv")
dat <- dat[7:8,]
...
#Figure 3, Panel 1#
###################
tiff(filename = "DiD_Results_Panel_1.tiff", type = "cairo" ,  res=150, height = 5, width = 2.5, units = "in")
ggplot(dat,aes(x = group2, y = p, ymin = low, ymax = high, group=group, color=group)) + 
  ...
  ggtitle("[1] \u0394 HyADS continuous\nannual effect estimates") +
  theme(plot.title = element_text(size = 14)) +
  theme(plot.title = element_text(hjust = 0.5))
dev.off()

dat<-read_csv("did_zip_results_ggplot_cats.csv")
dat$group2<-reorder(dat$group2,dat$ord)
dat$group <- factor(dat$group, levels = c("Unadjusted", "Adjusted"))
###################
#Figure 3, Panel 2#
###################
tiff(filename = "DiD_Results_Panel_2.tiff", type = "cairo" ,  res=150, height = 5, width = 3, units = "in")
ggplot(dat,aes(x = group2, y = p, ymin = low, ymax = high, group=group, color=group)) + 
  geom_pointrange(position = position_dodge(width = .3), size = 1.1, fatten = 1.5) + 
  geom_hline(yintercept = 0, linetype = "dotted") + 
...
#Figure 3, Panel 3#
###################
tiff(filename = "DiD_Results_Panel_3.tiff", type = "cairo" ,  res=150, height = 5, width = 2.75, units = "in")
ggplot(dat,aes(x = group2, y = p, ymin = low, ymax = high, group=group, color=group)) + 
  geom_pointrange(position = position_dodge(width = .3), size = 1.1, fatten = 1.5) + 
  geom_hline(yintercept = 0, linetype = "dotted") + 
  scale_y_continuous("\u0394 quarterly asthma hospitalizations/ERVs", limits = c(-8.1,0)) + 
  scale_color_manual("", values = c("#404688FF", "#20A486FF")) +
  xlab("")+ theme_minimal(base_size = 14) +
  theme(legend.title=element_blank(),legend.margin=margin(0,0,0,0),
        legend.box.margin=margin(0,0,0,-5),strip.placement = "outside")+
  theme(legend.position = "none") +
  ggtitle("[3] \u0394 HyADS binary\nquarterly effect estimates") +
  theme(plot.title = element_text(hjust = 0.5))  +
  theme(plot.title = element_text(size = 14)) 
dev.off()
...
#Figure 3, Panel 4#
###################
tiff(filename = "DiD_Results_Panel_4.tiff", type = "cairo" ,  res=150, height = 5, width = 3, units = "in")
ggplot(dat, aes(x = group2, y = p, ymin = low, ymax = high, group=group, color=group)) + 
  geom_pointrange(position = position_dodge(width = .3), size = 1.1, fatten = 1.5) + 
  geom_hline(yintercept = 0, linetype = "dotted") + 
