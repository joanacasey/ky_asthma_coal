...
spatial.change.from.year.g <- function( unit.use = 'all',
  ...
  use.dt2 <- use.dt[, .( hyads = mean( exposure)), by = sum.by]
  ...
  if( time == 'month'){
    ...
    fw <- facet_wrap( ~ time + year.x, ncol = 4, labeller = label_bquote( atop( .(month.name[as.numeric( as.character( time))]),
      ...
      fw <- facet_wrap( ~ time + year.x, ncol = 4, labeller = label_bquote( .(month.name[as.numeric( as.character( time))])))
  }
  if( time == 'quarter')
  ...
  ggchange <- ggplot( dt.plot) +
    ...
    geom_label( data = pct.plot,
                aes( label = paste( 'mean reduction =', ifelse( mean.p > 0,
                                                      paste0( '-', percent( mean.p, accuracy = 2)),
                                                      percent( abs( mean.p), accuracy = 2)))),
...
# zcta_shapefile <- './data/cb_2016_us_zcta510_500k.shp'
# crosswalk_csv <- "./data/Zip_to_ZCTA_crosswalk_2015_JSI.csv"
zcta_shapefile <- 'cb_2016_us_zcta510_500k.shp'
...
## ====================================================== ##
# timeseries.dt <- fread( "./data/louisville_exposure2_share.csv")
...
timeseries.dt.m2 <- timeseries.dt.m[, sum( value), by = .( ZIP, uID, variable, date.month, quarter)]
setnames( timeseries.dt.m2, 'V1', 'value')

# average across all ZIPs
timeseries.dt.m3 <- timeseries.dt.m2[, mean( value, na.rm = T), by = .( uID, variable, date.month, quarter)]
setnames( timeseries.dt.m3, 'V1', 'HyADS')

# sum by ZIP code, unit, and quarter
timeseries.dt.m4 <- timeseries.dt.m[, mean( value), by = .( uID, quarter, year)]
setnames( timeseries.dt.m4, 'V1', 'value')
dates.quart <- data.table( quarter = 1:4,
                           year = rep( 2012:2017, each = 4),
                           date.month = as.Date( paste( rep( 2012:2017, each = 4), seq( 1, 10, 3), '01', sep = '-')))

timeseries.dt.m4 <- merge( timeseries.dt.m4, dates.quart, by = c( 'quarter', 'year'))
...
Figure1A <- ggplot( timeseries.dt.m4,
                    aes( y = value, x = as.Date( date.month), fill = uID)) + 
  theme_bw() + 
  geom_area( position = 'stack') + #data = timeseries.dt.m2, 
...
Figure1Apres <- ggplot( timeseries.dt.m3,
